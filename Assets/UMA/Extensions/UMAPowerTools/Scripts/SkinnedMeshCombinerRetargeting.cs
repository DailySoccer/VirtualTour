...
namespace UMA.PowerTools
{
	...
	public static class SkinnedMeshCombinerRetargeting
	{
		...
		public static void CombineMeshes(UMAMeshData target, CombineInstance[] sources, Matrix4x4[] inverseTargetBoneMatrixes)
		{
			int vertexCount = 0;
			...
			AnalyzeSources(sources, subMeshTriangleLength, ref vertexCount, ref meshComponents);
			...
			foreach (var source in sources)
			{
				...
				{
					context.ProcessVertex(ref source.meshData.unityBoneWeights[i], i, ref boneWeights[vertexIndex + i], ref vertices[vertexIndex + i], ref normals[vertexIndex + i], ref tangents[vertexIndex + i]);
				}
		...
		private static void AnalyzeSources(CombineInstance[] sources, int[] subMeshTriangleLength, ref int vertexCount, ref MeshComponents meshComponents)
		{
			for (int i = 0; i < subMeshTriangleLength.Length; i++)
			{
				subMeshTriangleLength[i] = 0;

		...
		public override void OnInspectorGUI() 
		{
			...
			if (EditorApplication.isPlaying)
			{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					...
					if (dynamicAvatars[i] != null && dynamicAvatars[i].umaData.umaRoot != null)
					{
						allowHide = true;
					}
					if (!allowSaveRecipe && dynamicAvatars[i].umaData.umaRoot != null && oldRecipes[i] == null)
					{
						allowSaveRecipe = true;
					}
					if (!allowSavePrefab && dynamicAvatars[i] != null && dynamicAvatars[i].umaData.umaRoot != null && string.IsNullOrEmpty(AssetDatabase.GetAssetPath(dynamicAvatars[i].umaData)))
					{
				...
				for (int i = 0; i < targets.Length; i++)
				{
					if (dynamicAvatars[i].umaData.umaRoot != null)
					{
				...
				if( allowSaveRecipe )
				{
					if( GUILayout.Button("Save Recipe") )
					{
						...
						for (int i = 0; i < targets.Length; i++)
						{
							if (dynamicAvatars[i].umaData.umaRoot != null && oldRecipes[i] == null)
							{
				...
				if (allowSavePrefab)
				{
					if (GUILayout.Button("Create Prefab"))
					{
						for (int i = 0; i < targets.Length; i++)
						{
							if (dynamicAvatars[i] != null && dynamicAvatars[i].umaData.umaRoot != null && string.IsNullOrEmpty(AssetDatabase.GetAssetPath(dynamicAvatars[i].umaData)))
							{
			...
			GUILayout.BeginHorizontal();
			if (allowShow && UnityEngine.Application.isPlaying)
			{
				if (GUILayout.Button("Show"))
				{
					for (int i = 0; i < targets.Length; i++)
			...
			if (allowHide && UnityEngine.Application.isPlaying)
			{
				if (GUILayout.Button("Hide"))
				{
					for (int i = 0; i < targets.Length; i++)

...
namespace UMA.PowerTools
{
	public class UMALODController : MonoBehaviour
	{
		...
		public class LODEntry
		{
			...
			private void UpdateList(LinkedList<UMALODAvatar> list)
			{
				...
				while (iterator != null)
				{
					...
					if (distance > leaveRangeSquared)
					{
						...
						while (nextLOD != null)
						{
							if (distance < nextLOD.enterRangeSquared)
							{
								lowerLOD.Enter(autoLoaderNode.Value);
								break;
							}
			...
			{
				autoloader.umaData.myRenderer.quality = skinQuality;
				autoloader.umaData.animator.enabled = !disableAnimations;

...
public class UMALODManager : MonoBehaviour
{
	...
	private OverlayData ProcessUMAOverlay(OverlayData overlayData, OverlayData reference, int LOD)
	{
		...
		if (reference != null && reference.overlayName != overlayData.overlayName)
		{
			reference = null;
		}
		if (reference == null)
		...
		{
			if (overlayData.useAdvancedMasks)
			{
				reference.channelAdditiveMask = overlayData.channelAdditiveMask;
				reference.channelMask = overlayData.channelMask;
			}
			else
			{
				reference.color = overlayData.color;
			}
		}
		return reference;
	}
	...
	public class OptimizedLODManager
	{
		public class OptimizedConversionEntry
		{
			...
			public OverlayData InstantiateOverlay(int LOD, UMAContext context, OverlayData reference)
			{
				...
				if (reference != null)
				{
					if (UMASkeleton.StringToHash(reference.overlayName) == knownDestination)
					{
						return reference;
					}
				}

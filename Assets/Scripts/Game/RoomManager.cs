...
public class RoomManager : Photon.PunBehaviour {
	...
	private IEnumerator EnterPlayer(RoomDefinition roomOld, Player player) {
		...
		if (portal != null) {
			...
			if (entrada != null) {
				if (player != null) {
					player.UMAAvatar.transform.position = entrada.position;
					player.UMAAvatar.transform.rotation = entrada.rotation;
				}
		...
		if (portal == null) {
			...
			if (player != null) {
				...
				player.UMAAvatar.transform.position = Vector3.zero;
				player.UMAAvatar.transform.rotation = Quaternion.identity;
			}
		}
